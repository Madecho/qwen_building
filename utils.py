...
def make_mesh(ground_points):
    tri = Delaunay(ground_points[:, :2])
    surface = o3d.geometry.TriangleMesh()
    surface.vertices = o3d.utility.Vector3dVector(ground_points)
    surface.triangles = o3d.utility.Vector3iVector(tri.simplices)
    surface.compute_vertex_normals()
    return surface


def discretize(t, continuous_range, num_discrete=128):
    is_ndarray = False
    ...
    res = t.round().long().clamp(min=0, max=num_discrete - 1)
    if is_ndarray:
        res = res.numpy().astype(np.int64)
    return res


def undiscretize(t, continuous_range, num_discrete=128):
    is_ndarray = False
    if isinstance(t, np.ndarray):
        is_ndarray = True
        t = torch.Tensor(t)
